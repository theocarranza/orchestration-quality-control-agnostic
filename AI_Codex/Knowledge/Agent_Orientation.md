---
type: orientation
tags: [knowledge, orientation]
created: 2026-07-16
---

# Agent Orientation

Use this note as the entry point for agents working in this vault.

```mermaid
flowchart TD
  START["Start of session"] --> BOOT["Read this orientation"]
  BOOT --> SESS["Newest Agent_Sessions note"]
  SESS --> KNOW["Knowledge/Documentation<br/>docs vs ledger"]
  KNOW --> WORK["Do the work"]
  WORK --> LOG["Update the session note"]
```

## Commands

| Command | Purpose |
| --- | --- |
| `/codex-workflow:codex-init-workspace` | Scaffold the CLAUDE.md tree. |
| `/codex-workflow:codex-init-vault` | Scaffold this vault skeleton. |
| `/codex-workflow:codex-init-rules` | Drop starter `.agent/rules/*.md` templates. |
| `/codex-workflow:codex-mine-bases` | Backfill frontmatter + Base dashboards (software-project). |
| `/codex-workflow:codex-query-vault` | Read-only live query of the vault via the Obsidian CLI. |
| `/codex-workflow:codex-canvas-map` | Generate an Architecture Canvas relationship map. |
| `/codex-workflow:codex-research-ingest` | Ingest a URL into a source-stamped reference note. |
| `/codex-workflow:codex-vault-lint` | Audit the vault against its archetype spec. |

## Conventions

- Status is encoded by folder for ticket notes.
- Frontmatter stores identity and classification.
- Keep project vocabularies for `type`, `area`, and `stack` small and stable.

## Knowledge Index

| Note | Role |
| --- | --- |
| [[Agent_Orientation]] | This orientation note |
| [[Documentation]] | Audience split (`docs/` vs this ledger) and visual-docs rule |
