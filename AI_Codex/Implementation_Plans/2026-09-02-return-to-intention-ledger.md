...
| Step | Deliverable | Executor model | Attempts | Validator verdict | Root decision | Status | Commit |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1.1 | ADR status lines and amendments (0013, 0008, 0010, 0012, 0005) | | | | | pending | |
| 1.2 | `input.schema.json` three operations, `decision`, `language`, `stop_after_prepare` | | | | | pending | |
| 1.3 | SKILL.md + READMEs: interview statement replaces approval wording | | | | | pending | |
| 1.4 | Core and author `evals.json` rewritten | | | | | pending | |
