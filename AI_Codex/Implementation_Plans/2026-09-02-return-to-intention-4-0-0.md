...
envelope. The Orchestrator agent loops _`oqc.py next` → `oqc.py compile` →
spawn worker with the compiled prompt → `oqc.py gate`_; it never authors,
...
and `derive_finding_id`), `lint_rules.py`, `checkpoint_state.py`,
