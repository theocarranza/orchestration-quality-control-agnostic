---
description: Quality-control rules for generator sources (rules, workflows, prompts) that produce E2E artifacts
globs:
  - "**/rules/rules-*.md"
...
# Rule: E2E Generator-Source Quality Control

Apply this rule when validating a generator source — a rules file, workflow
file, or prompt that instructs an agent to produce E2E artifacts — for whether
following it would cause the produced artifacts to break the artifact quality
rules.

## Scope

- Applies to rules files, workflow files, and prompt files whose content
  instructs the creation of domain contracts, test plans, blueprints, flows,
  subflows, data files, or fingerprints.
- Does not apply to the finished artifacts themselves (judge those against
  `rules-e2e-artifact-quality-control.md`), to a rules file's own authoring
  form (`rules-e2e-rules-quality-control.md`), or to a workflow's own authoring
  form (`rules-e2e-workflow-quality-control.md`).

## Required Context

- Read the target generator source in full before judging it.
- Read `rules-e2e-artifact-quality-control.md` as the set of packaged rules a
  produced artifact must satisfy.
...
rule for a type this generator produces.

### G3 — Gap

Flag a generator responsible for an artifact type covered by a packaged rule,
when the text never requires or protects that rule and nothing in the text
prevents the resulting violation.
