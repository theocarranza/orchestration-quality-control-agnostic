...
- Does not apply to single-stage worker documents, finished E2E artifacts
  (domain, test plan, blueprint, flow, subflow, data, fingerprint), or general
  production code.
- An orchestrator workflow authored against `workflows-template.md` is also
  subject to `rules-e2e-workflow-quality-control.md`; this rule set adds the
