...
- Does not apply to rules files, finished E2E artifacts (domain, test plan,
  blueprint, flow, subflow, data, fingerprint), or general production code.
...
- For a generator source, state whether the finding is a contradiction (the text
  pushes toward a violation) or a gap (the text omits a needed protection).
