#!/usr/bin/env python3
"""Create and clear adapter-only authorizations for approved OQC changes."""
...
def authorization_path(checkpoint_path: Path, run_id: str) -> Path:
    return checkpoint_path.parent / f"cursor-authorization-{run_id}.json"
...
def main() -> None:
    ...
    try:
        if args.command == "create":
            path = create(checkpoint_path, args.approved_ids)
            result = {"authorization_path": str(path)}
        else:
            path = clear(checkpoint_path)
            result = {"cleared_authorization_path": str(path)}
    except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as error:
        print(json.dumps({"status": "blocked", "stage": "cursor_authorization", "detail": str(error)}))
        raise SystemExit(2)
