#!/usr/bin/env python3
"""Build a reproducible Cursor marketplace from the canonical OQC package."""
...
def build(output: Path) -> Path:
    ...
    shutil.copytree(
        _adapter_root() / "skills" / "oqc-upgrade",
        plugin_root / "skills" / "oqc-upgrade",
        ignore=_ignore,
    )
    shutil.copytree(
        _adapter_root() / "skills" / "oqc-author",
    ...
    _write_json(plugin_root / ".cursor-plugin" / "plugin.json", manifest)
    ...
    _write_json(output / ".cursor-plugin" / "marketplace.json", marketplace)
    shutil.copy2(_adapter_root() / "install_cursor.py", output / "install_cursor.py")
    ...
    try:
        with zipfile.ZipFile(temporary, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
            for path in sorted(item for item in marketplace_root.rglob("*") if item.is_file()):
                relative = Path("cursor-marketplace") / path.relative_to(marketplace_root)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default=str(_repo_root() / "dist" / "cursor-marketplace"))
    parser.add_argument("--zip", dest="zip_path", default=str(_repo_root() / "dist" / f"{PLUGIN_NAME}-cursor-{VERSION}.zip"))
...
def main() -> None:
    ...
    print(f"Built Cursor marketplace: {output}")
    if not args.no_zip:
        print(f"Built Cursor release: {write_zip(output, Path(args.zip_path))}")
