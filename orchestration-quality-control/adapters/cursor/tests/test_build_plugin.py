...
def _load_builder():
    spec = importlib.util.spec_from_file_location("cursor_build_plugin", BUILDER_PATH)
...
class BuildPluginTest(unittest.TestCase):
    def setUp(self):
        ...
        self.output = self.root / "cursor-marketplace"

    def tearDown(self):
        self.temp.cleanup()

    def test_build_has_expected_cursor_plugin_marketplace_and_components(self):
        ...
        marketplace = json.loads((self.output / ".cursor-plugin" / "marketplace.json").read_text(encoding="utf-8"))
    ...
    def test_build_injects_cursor_overlay_without_mutating_canonical_skill(self):
        ...
        self.assertIn("## Cursor adapter execution", text)
    ...
    def test_zip_is_reproducible_and_rooted(self):
        ...
        self.assertTrue(all(name.startswith("cursor-marketplace/") for name in names))
