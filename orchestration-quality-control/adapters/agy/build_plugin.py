#!/usr/bin/env python3
"""Build a reproducible Antigravity (AGY) plugin and marketplace from the canonical OQC package."""
...
def build(output: Path) -> Path:
    ...
    entrypoints = _package_root() / "entrypoints"
    for entry in ("orchestration-upgrade", "orchestration-author", "orchestration-engine"):
        source = entrypoints / entry
        if source.is_dir():
            shutil.copytree(source, plugin_root / "skills" / entry, ignore=_ignore)
    ...
    shutil.copytree(_adapter_root() / "rules", plugin_root / "rules", ignore=_ignore)
    shutil.copytree(_adapter_root() / "hooks", plugin_root / "hooks", ignore=_ignore)
    shutil.copy2(_adapter_root() / "hooks" / "hooks.json", plugin_root / "hooks.json")
    ...
    _write_json(output / "plugins.json", plugins_registry)
    shutil.copy2(_adapter_root() / "install_agy.py", output / "install_agy.py")
    ...
    try:
        with zipfile.ZipFile(temporary, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
            for path in sorted(item for item in marketplace_root.rglob("*") if item.is_file()):
                relative = Path("agy-marketplace") / path.relative_to(marketplace_root)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default=str(_repo_root() / "dist" / "agy-marketplace"))
    parser.add_argument(
        "--zip", dest="zip_path", default=str(_repo_root() / "dist" / f"{PLUGIN_NAME}-agy-{VERSION}.zip")
    )
...
def main() -> None:
    ...
    print(f"Built Antigravity (AGY) marketplace: {output}")
    if not args.no_zip:
        print(f"Built Antigravity (AGY) release: {write_zip(output, Path(args.zip_path))}")
