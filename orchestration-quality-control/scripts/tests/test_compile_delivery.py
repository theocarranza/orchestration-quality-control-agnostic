...
class ConventionsTest(unittest.TestCase):
    """The engine must extend the tree that exists, not build a second one beside it.

    Found by the independent content review during the 2026-09-09 repeat trial:
    the emitted package was structurally correct but generic, carrying none of the
    client's actual layout, configuration, selector choices, or recorded unsafe
    command. These facts live in the specification, so the generator stays generic.
    """
    ...
    def test_the_shipped_specification_carries_the_real_conventions(self):
        conventions = _spec()["conventions"]
        self.assertEqual(conventions["config_file"], "e2e_test/config.yaml")
        self.assertEqual(conventions["flow_glob"], "modules/**/*.flow.yaml")
        self.assertIn("appId: ${APP_ID}", conventions["flow_header"])

    def test_the_flow_template_shows_the_required_header(self):
        files = self._build(_spec())
        self.assertIn("appId: ${APP_ID}", files["templates/flow.md"])

    def test_the_flow_template_names_the_collecting_glob(self):
        files = self._build(_spec())
        self.assertIn("modules/**/*.flow.yaml", files["templates/flow.md"])

    def test_the_recorded_unsafe_command_reaches_both_the_rules_and_the_template(self):
        files = self._build(_spec())
        self.assertIn("hideKeyboard", files["rules/rules-engine.md"])
        self.assertIn("hideKeyboard", files["templates/flow.md"])

    def test_the_unsafe_command_says_what_happens_and_what_to_do_instead(self):
        rules = self._build(_spec())["rules/rules-engine.md"]
        self.assertIn("never recovers", rules)
        self.assertIn("accessibility identifier", rules)

    def test_the_selector_preference_reaches_the_rules(self):
        rules = self._build(_spec())["rules/rules-engine.md"]
        self.assertIn("stable accessibility identifier", rules)

    def test_the_scenario_layout_reaches_the_plan_template(self):
        plan = self._build(_spec())["templates/test-plan.md"]
        for expected in ("test-plan.md", "blueprint.md", "execution-report.md"):
            self.assertIn(expected, plan)

    def test_the_environment_variables_reach_the_rules(self):
        rules = self._build(_spec())["rules/rules-engine.md"]
        for name in ("APP_ID", "ACCOUNT_EMAIL"):
            self.assertIn(name, rules)

    def test_the_runner_commands_reach_the_rules(self):
        self.assertIn("npm run test:e2e:android", self._build(_spec())["rules/rules-engine.md"])

    def test_the_rules_forbid_building_a_second_tree(self):
        self.assertIn("Never create a second one", self._build(_spec())["rules/rules-engine.md"])

    def test_conventions_are_optional(self):
        spec = _spec()
        del spec["conventions"]
        files = self._build(spec)
        self.assertIn("templates/flow.md", files)
        self.assertNotIn("hideKeyboard", files["rules/rules-engine.md"])

    def test_changing_a_convention_changes_the_emitted_package(self):
        baseline = self._build(_spec())
        spec = _spec()
        spec["conventions"]["flow_glob"] = "cases/**/*.yaml"
        self.assertNotEqual(baseline["templates/flow.md"], self._build(spec)["templates/flow.md"])

    def test_an_unsafe_command_missing_its_consequence_is_refused(self):
        spec = _spec()
        spec["conventions"]["unsafe_commands"] = [{"command": "hideKeyboard"}]
        with self.assertRaises(Blocked):
            client_spec.validate(spec)

    def test_an_unknown_convention_field_is_refused(self):
        spec = _spec()
        spec["conventions"]["invented"] = "no"
        with self.assertRaises(Blocked) as caught:
            client_spec.validate(spec)
        self.assertIn("unknown field", caught.exception.detail)
