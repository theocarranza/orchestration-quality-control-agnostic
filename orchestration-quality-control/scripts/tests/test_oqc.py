...
from kernel_specs import AgentSpec, Envelope, TaskDag
...
class VerifyValidMailboxTest(unittest.TestCase):
    def test_verify_of_a_valid_drive_run_returns_the_live_state(self):
        ...
        adapter = FakeAdapter(SCRIPT_A)
        live_state = drive(DAG, adapter, mailbox, AGENT_SPECS, MAX_ATTEMPTS, run_id=RUN_ID)
        self.assertEqual(verify(mailbox), live_state)

    def test_verify_docstring_disclaims_cryptographic_coverage(self):
        # Documentation-truth guard: this function must not be described
        # as something it is not.
        doc = verify.__doc__.lower()
        self.assertIn("not", doc)
        self.assertIn("cryptographic", doc)
    ...
    def test_a_request_in_flight_with_no_result_yet_still_verifies_clean(self):
        ...
        # kind of violation.
        mailbox = Mailbox()
        adapter = FakeAdapter({("task-a", 1): {"outcome": "passed"}})
        adapter.spawn(mailbox, run_id=RUN_ID, task_id="task-a", attempt=1,
                      agent_id="worker-a", brief={})
        request_only = _corrupt_mailbox([mailbox.read_all()[0]])  # drop the result
        state = verify(request_only)
        self.assertEqual(status_of(state, "task-a"), "running")
...
class VerifyDuplicateResultTest(unittest.TestCase):
    ...
    def test_a_duplicate_result_overriding_a_real_failure_is_blocked(self):
        ...
        second_result = _envelope(
            envelope_id="env-3", kind="result", sender="agent:worker-a",
            recipient="orchestrator",
            payload={"task_id": "task-a", "attempt": 1, "outcome": "passed"},
        )
        # Built through the ordinary, un-bypassed public API -- this is
        # not a tampering scenario, it is what a flaky transport's
        # duplicated callback (or an outright forged second report) looks
        # like on the wire.
...
class VerifyDeletionTest(unittest.TestCase):
    def test_deleting_a_request_leaves_its_result_orphaned_and_blocked(self):
        legit = Mailbox()
        adapter = FakeAdapter({("task-a", 1): {"outcome": "passed"}})
        adapter.spawn(legit, run_id=RUN_ID, task_id="task-a", attempt=1,
                      agent_id="worker-a", brief={})
        _request, result = legit.read_all()
...
class CliTest(unittest.TestCase):
    ...
    def test_verify_subcommand_on_a_tampered_file_exits_two_with_a_blocked_payload(self):
        ...
        line = Envelope.from_dict({
            "schema_version": 1, "envelope_id": "env-1", "run_id": RUN_ID,
            "sender": "orchestrator", "recipient": "root", "kind": "status",
            "payload": {"phase": "discovery"}, "created_at": "2026-09-05T12:00:00Z",
        }).to_json()
