    ...
    workers = {
        first_spec.agent_id: {
            ...
            "prompt": (
                ...
                "The result is a JSON object with exactly these fields: task_id (string), "
                "attempt (integer, 1 or greater), outcome (either 'passed' or 'failed'), and "
                "artifact (a nonblank artifact string); critique (string) and question (an "
                "object with question_id and prompt) are included only on a failed result; "
                "no other field is permitted. Everything needed to answer is already in this "
                "prompt: no file, schema, or tool is required or available, so answer directly "
        ...
        second_spec.agent_id: {
            ...
            "prompt": (
                ...
                "never include critique or question. "
                "The result is a JSON object with exactly these fields: task_id (string), "
                "attempt (integer, 1 or greater), outcome (either 'passed' or 'failed'), and "
                "artifact (a nonblank artifact string); critique (string) and question (an "
                "object with question_id and prompt) are included only on a failed result; "
