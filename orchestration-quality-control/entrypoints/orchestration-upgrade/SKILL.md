...
description: >
  ...
  compares it with the isolated three-agent OQC reference architecture,
  drafts a complete version plus ARCHITECTURE.md diagrams, presents literal
  changes, applies only an atomic approval, and verifies the result. Use
  when the user asks to upgrade, redesign, version, normalize, or template
...
3. The root session owns all UI: mechanism path, manifest confirmation,
   profile/language, apply mode/paths, and atomic approve/decline decision.
