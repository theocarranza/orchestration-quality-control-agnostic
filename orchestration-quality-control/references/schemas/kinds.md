...
generator-contradiction | workflow | orchestrator` mixed core and profile kinds in one
flat enum).
...
uses the profile's own `id` as the namespace. For example, the `example-pipeline` profile
declares `example-pipeline/artifact` for its pipeline-artifact findings.
